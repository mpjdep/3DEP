...
%
% OUTPUT:
%   \Full_Data.xlsx: Contains all the datapoints from all technical
...
%% Initialize Excel in background
...
disp(['Data compiled from ', num2str(numFiles), ' repeats']);
        sortedfullData = sortrows(fullData, 1);
        
outputFile = fullfile(folderPath, 'Full_Data.xlsx');
fullDataTable = array2table(sortedfullData, 'VariableNames', {'Frequency', 'Intensity', 'Notes'});
disp('Data sorted');
...
% Connect to an existing Excel instance or open Excel if it's not already running
